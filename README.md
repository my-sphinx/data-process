...
一个逐步演进的数据处理工具仓库。当前已实现 `数据清洗` 和 `数据去重`。

执行清洗或去重时，终端会通过 `tqdm` 实时显示多阶段进度条；任务完成后会继续输出最终统计信息。`csv` 默认展示 `统计总行数 -> 分块处理 -> 写出结果`，`Excel` 展示 `读取文件 -> 执行处理 -> 写出结果`。清洗阶段完成时，进度条尾部会附带 `总数 / 删除 / 保留` 摘要，以及 `空行 / 符号 / 表情 / 乱码` 的删除分布；去重阶段会展示 `总数 / 重复 / 保留 / 唯一值` 摘要。每次运行还会在输出文件同目录生成统一日志文件 `data-process.log`，写入阶段日志、错误信息和最终统计。
...
| 参数 | 是否必填 | 说明 | 支持的值 |
| --- | --- | --- | --- |
| `--action` | 是 | 指定要执行的功能。当前工具通过该参数选择不同处理动作。 | `clean`、`deduplicate` |
...
| `删除重复行数` | 基于目标列标准化后判定为重复并删除的行数。 |
| `标准化后唯一值数量` | 对目标列执行去首尾空格、压缩连续空白、大小写归一后，最终得到的唯一值数量。 |
| `去重后总行数` | 去重完成后保留下来的总行数。 |

## 日志文件

每次执行 `clean` 或 `deduplicate` 时，程序都会在输出文件同目录生成或追加写入 `data-process.log`。日志会记录：
